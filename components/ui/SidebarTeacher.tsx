...
const SidebarTeacher = () => {
  ...
  const teacherName = user ? `${user.firstName} ${user.lastName}` : "Professeur";
  const teacherInitials = user ? `${user.firstName?.[0] || ''}${user.lastName?.[0] || ''}` : "P";
          ...
          {Object.entries(groupedItems).map(([category, items]) => (
            <div key={category} className="p-3 space-y-1 border-t border-gray-100">
              <h3 className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">
                {category === "PÉDAGOGIE" ? "ORGANISATION PÉDAGOGIQUE" : category}
              </h3>
