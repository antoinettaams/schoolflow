...
const SidebarParent = () => {
  ...
  const parentInitials = user ? `${user.firstName?.[0] || ''}${user.lastName?.[0] || ''}` : "P";
  ...
  const navItems = [
    { label: "Tableau de Bord", href: "/dashboard/parent", icon: <FaUsers /> },
    { label: "Emplois du Temps", href: "/dashboard/parent/schedules", icon: <FaCalendarAlt />, category: "SUPERVISION" },
  ...
  const groupedItems = {
    "SUPERVISION": navItems.filter((i) => i.category === "SUPERVISION"),
