...
type ExamType = "Interrogation" | "Devoir" | "Composition" | "Projet";
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex-1 min-w-0">
                <CardTitle className="text-2xl sm:text-3xl font-bold truncate">Examens & Notes</CardTitle>
              </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex justify-center items-center border-b border-gray-100">
              <div className="font-bold text-gray-800 bg-blue-50/50 p-2 rounded-lg shadow-inner flex items-center gap-2 md:flex flex-col text-sm">
                ...
                <Badge className={`ml-2 text-xl py-1 px-3 ${getGradeColor(generalAverage)}`}>{generalAverage.toFixed(2)}</Badge>
              </div>
            </div>
                  ...
                  <TableBody>
                    {finalData.map((moduleData) => (
                      <TableRow key={moduleData.module} className="hover:bg-gray-50">
                        <TableCell className="font-medium sticky left-0 text-sm sm:text-base truncate">{moduleData.module}</TableCell>

                        <TableCell className="text-center font-medium text-gray-700 bg-gray-50/50 text-sm">{moduleData.coefficient}</TableCell>
                        ...
                        <TableCell className="text-center bg-blue-50/20 border-l border-gray-200">
                          <Badge className={`font-bold text-sm sm:text-base ${getGradeColor(moduleData.moduleAvg)}`}>{moduleData.moduleAvg.toFixed(2)}</Badge>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
